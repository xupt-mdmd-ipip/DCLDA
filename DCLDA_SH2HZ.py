...
def append_logs(mes):
    ...
    # 定义文件路径
    file_path = "log.txt"  # 这将在当前目录下创建一个名为 example.txt 的文件

    # 使用 with 语句打开文件，确保文件会被正确关闭
    with open(file_path, 'a', encoding='utf-8') as file:
        file.write(text_to_append)

def get_probs(data_loader, data_loader_t, feature_encoder):
...
def clean_sampling_epoch(labels, probabilities, output, testX, testY):
    ...
    label_errors_bool = cleanlab.filter.find_label_issues(labels=labels, pred_probs=probabilities,
                                                          filter_by='prune_by_class', n_jobs=1)

    ordered_label_errors = cleanlab.rank.order_label_issues(
        label_issues_mask=label_errors_bool,
...
def SH2HZ():
    ...
    try:
        with open('log.txt', 'w') as file:
            file.write('')  # 写入空字符串，相当于清空文件内容
        print("log.txt 文件已成功清空。")
    ...
    for iDataSet in range(nDataSet):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if (epoch >= train_num and epoch < epochs) and epoch % 20 == 0:
                ...
                probs = get_probs(train_loader_s, test_loader, feature_encoder)
                clean_datas, clean_labels, class_weights, clean_acc = clean_sampling_epoch(fake_label, probs, output, testX, testY)
            ...
            for i in range(1, num_iter):
                ...
                _, source2, _, _, output2 = feature_encoder(source_data0.cuda())
                _, source3, _, _, output3 = feature_encoder(source_data1.cuda())
                target_features, _, target1, target_outputs, target_out = feature_encoder(target_data.cuda())

                _, _, target2, t1, output4 = feature_encoder(target_data0.cuda())
                _, _, target3, t2, output5 = feature_encoder(target_data1.cuda())
                ...
                de_cl_loss_s = DCL.debiased_loss(output2, output3, batch_size=BATCH_SIZE, temperature=5.0, debiased=True,
                                                 tau_plus=0.05)
                # Loss Con_t
                de_cl_loss_t = DCL.debiased_loss(output4, output5, batch_size=BATCH_SIZE, temperature=2.0, debiased=True,
                                                 tau_plus=0.1)
                # Loss Occ
                import Advloss

                adversarial_loss_fn = Advloss.AdversarialLoss(gamma=3.0, max_iter=800, use_lambda_scheduler=True).cuda()
                domain_adv_loss = adversarial_loss_fn(source_features, target_features)



                loss = cls_loss + 0.01 * lambd * lmmd_loss + de_cl_loss_s + de_cl_loss_t + domain_adv_loss
                # Update parameters
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            print(
                'epoch {:>3d}:   cls loss: {:6.4f},lmmd loss:{:6f}, occ loss:{:6f},con_s loss:{:6f}, con_t loss:{:6f},acc {:6.4f}, total loss: {:6.4f}'
                .format(epoch, cls_loss.item(), lmmd_loss.item(), domain_adv_loss.item(), de_cl_loss_s.item(),
                        de_cl_loss_t.item(),
            ...
            if epoch % epochs == 0:
                ...
                if test_accuracy > last_accuracy:
                    ...
                    #            str("JCLDA_feature_encoder_" + "houston" + str(iDataSet) + ".pkl"))
    ...
    append_logs("train time per DataSet(s): " + "{:.5f}".format(train_end-train_start))

    append_logs("test time per DataSet(s): " + "{:.5f}".format(test_end-train_end))
    append_logs("average OA: " + "{:.2f}".format( OAMean) + " +- " + "{:.2f}".format( OAStd))
    append_logs("average AA: " + "{:.2f}".format(100 * AAMean) + " +- " + "{:.2f}".format(100 * AAStd))
    append_logs("average kappa: " + "{:.4f}".format(100 *kMean) + " +- " + "{:.4f}".format(100 *kStd))
    append_logs("accuracy for each class: ")
    ...
    for i in range(len(acc)):
        ...
        append_logs('{}:{}'.format(i, acc[i]))
        if acc[i] > acc[best_iDataset]:
            best_iDataset = i
    print('best acc all={}'.format(acc[best_iDataset]))
    append_logs('best acc all={}'.format(acc[best_iDataset]))
    ...
    image_path = "classificationMap/housotn18.png"
    utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24, image_path)
    return image_path
